...
- git tag, tagging a commit upon which the binaries are built
- GitHub Release attached to that git tag
- upload x86 Linux and x86 MacOS binaries to that GitHub Release
- CHANGELOG.md updates:
  - copy relevant section from CHANGELOG.md to GitHub Release notes
  - update headers: replace old `# Unreleased` header, with `# <release dfx version>`
-
...
Release process is driven by the DFINITY foundation SDK team. Future work needs to be done to separate the build and release process to run fully on open systems.
